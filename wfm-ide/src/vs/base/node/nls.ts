...
export async function resolveNLSConfiguration({ userLocale, osLocale, userDataPath, commit, nlsMetadataPath }: IResolveNLSConfigurationContext): Promise<INLSConfiguration> {
	...
	// OSS / dev builds may omit `product.commit`; use a stable segment for language-pack cache dirs.
	const commitSegment = commit || 'development';
	...
	try {
		...
		const commitLanguagePackCachePath = join(globalLanguagePackCachePath, commitSegment);
